  ...
  const renderFolderTree = (folderList: any[], level = 0) => {
    return folderList.map((folder) => (
      <div key={folder.id} className="w-full">
        ...
        {folder.children && folder.children.length > 0 && expandedFolders.includes(folder.id) && (
          <div className="ml-4">{renderFolderTree(folder.children, level + 1)}</div>
        )}
      </div>
    ));
  };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-96 max-h-96 overflow-y-auto">
            ...
            <div className="space-y-3">
              {selectedDocument.versions.map((version: any, index: number) => (
